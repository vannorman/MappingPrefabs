...
public class EditorTools : Editor {
   ...
   [MenuItem("Space Frog/Fix Missing Monos")]
   public static void FixMissingMonos()
   {
       bool clear = true;
       Transform[] go = Selection.activeGameObject.GetComponentsInChildren<Transform>(); 
       ...
       if (clear) {
           Debug.Log("<color=green><b>All clear!</b></color>");

       }
   }
   
   
   [MenuItem("Edit/Editor Tools/Arrange Objects In Circle")]
   public static void ArrangeObjectsInCircle(){
       for (int i = 0; i < Selection.transforms.Length; i++)
       {
           Transform t = Selection.transforms[i];
           t.Rotate(Vector3.up * 10);

       }
   }


   [MenuItem("Edit/Editor Tools/Add Forward Facing Gun")]
   public static void AddForwardFacingGun(){
       foreach (GameObject o in Selection.gameObjects)
       {
           GameObject t = new GameObject("gun fwd");
           t.transform.SetParent(o.transform);
           t.transform.localPosition = new Vector3(0,1.3f,0);
           t.transform.localRotation = Quaternion.Euler(new Vector3(270, 0, 0));
       }   
   }
   ...
   [MenuItem("Edit/Editor Tools/Asteroids")]
   public static void Asteroids(){
       List<GameObject> toD = new List<GameObject>();
       foreach (GameObject o in FindObjectsOfType<GameObject>())
       {
        
           if (o.name == "test_asteroid3_LOD0"){
               foreach (Transform t in o.transform)
               {
                   t.SetParent(o.transform.parent);
               }
               toD.Add(o);

           }
       }
       foreach (GameObject o in toD)
       {
           DestroyImmediate(o);
       }
   }
	...
	[MenuItem("Edit/Sorting Tools/Sort Transform By Name")]
	public static void SortTransformByName(Transform parent) {
		List<string> names = new List<string> ();
			

//		Dictionary<string,Transform> data = new Dictionary<Transform,string> ();
		...
		foreach (string s in names) {
			parent.Find (s).SetParent (newParent);
		}
		newParent.gameObject.name = parent.name;
	}
...
//		Vector3 center = Vector3.zero;
       Vector3 center = GetCenterOfObjects(Selection.gameObjects);
...
//      newParent.transform.position = middlestCenter;
   ...
   static Vector3 GetCenterOfObjects(GameObject[] objects){
       Vector3 center = Vector3.zero;
...
//			t.name += " moveme? ";
	...
	#else
	[MenuItem("Edit/Editor Tools/Toggle Gameobject Active %1")]
	#endif
	...
	[MenuItem("Edit/Editor Tools/Set Vector3.one Scale %S")]
...
//
//	[MenuItem("Edit/Editor Tools/Enable Vive Controls")]
//	public static void EnableViveControls(){
//		foreach(StartingPlayerObject so in Resources.FindObjectsOfTypeAll<StartingPlayerObject>()){
//			if (so.type == StartingPlayerObject.PlayerType.Desktop){
...
//	}
//
//	[MenuItem("Edit/Editor Tools/Enable Desktop Controls")]
//	public static void EnableDesktopControls(){
//		foreach(StartingPlayerObject so in Resources.FindObjectsOfTypeAll<StartingPlayerObject>()){
//			if (so.type == StartingPlayerObject.PlayerType.Desktop){
//				so.gameObject.SetActive(true);
//			} else {
